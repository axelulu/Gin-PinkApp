...
// RecommendList 首页文章列表
func RecommendList(p *models.PostRecommendList, userId int64) (home map[string]interface{}, err error) {
	home = make(map[string]interface{}, 2)
	categorys, err := mysql.GetCategoryList(1000)
	// 构造推荐分类
	var category []*models.Category
	var recommend = models.Category{
		CategorySlug: 1,
	...
	category = append(category, &recommend)
	home["category"] = append(category, categorys...)
	var recoPosts []*models.Post
	err, recoPosts = grpc.GetRecommendPost(p, userId)
	if err != nil {
		return nil, err
	}
	var postDetails []*models.PostDetail
	postDetails = GetPostDetail(recoPosts)
	home["post"] = postDetails
	home["banner"] = postDetails
	return
}
...
// RecommendPostList 根据分类获取文章列表
func RecommendPostList(p *models.PostRecommendList, userId int64) (recoPosts []*models.Post, err error) {
	err, recoPosts = grpc.GetRecommendPost(p, userId)
	return
}
